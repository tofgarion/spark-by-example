...
The ~Search_Lower_Bound~ algorithm searches for the lower index at which a value can be found in a sorted array.
...
- ~A~ should be sorted.
- ~A'Last~ must be less than A'Last since the result can be ~A'Last+1~.
...
*** The sepcification of Search_Lower_Bound
...
At aech iteration, the range that contains the sought-after result is narrowed down. Note that the ~middle~ is calculated so as to avoid potential overflows.
