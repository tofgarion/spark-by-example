...
*** The Constant_Range and Has_Constant_Subrange predicates
    ...
    #+INCLUDE: "../../../spec/has_constant_subrange_p.ads" :range-begin "function Constant_Range_From_Location" :range-end "\s-*(\([^()]*?\(?:\n[^()]*\)*?\)*)\s-*\([^;]*?\(?:\n[^;]*\)*?\)*;" :src ada :lines "7-17"
    ...
    #+INCLUDE: "../../../spec/has_constant_subrange_p.ads" :range-begin "function Has_Constant_Subrange" :range-end "\s-*(\([^()]*?\(?:\n[^()]*\)*?\)*)\s-*\([^;]*?\(?:\n[^;]*\)*?\)*;" :src ada :lines "18-25"

*** Specification of Search_N

    The specification of ~Search_N~ is rather easy to express:

    #+INCLUDE: "../../../non-mutating/search_n_p.ads" :range-begin "function Search_N" :range-end "\s-*(\([^()]*?\(?:\n[^()]*\)*?\)*)\s-*\([^;]*?\(?:\n[^;]*\)*?\)*;" :src ada :lines "9-30"
...
*** Implementation of Search_N
    ...
    #+INCLUDE: "../../../non-mutating/search_n_p.adb" :range-begin "function Search_N" :range-end "end Search_N;" :src ada :lines "6-38"
