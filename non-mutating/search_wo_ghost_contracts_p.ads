...
package Search_Wo_Ghost_Contracts_P with
     Spark_Mode is

   function Search (A : T_Arr; B : T_Arr) return Option with
     Pre            => A'Last < Positive'Last and then B'First <= B'Last,
     Post =>
       (If B'Length = 0 Or Else A'Length < B'Length Then not Search'Result.Exists
        Else
       (If (for some J in A'First .. A'Last - B'Length + 1 =>
           A (J .. J - 1 + B'Length) = B) Then Search'Result.Exists Else Not Search'Result.Exists));
     --  Contract_Cases =>
