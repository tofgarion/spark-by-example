...
-The post conditions are expressed through 4 contract cases:
    I. if B is empty, then there is no index returned
   II. if A is shorter than B, then again there is no index returned
  III. A is longer than B and there is a subsequence in A equal to
  B. Then 
   ...
   IV. Otherwise there is no index returned.
