...
2) The first element of the heap remains constant after the first swap. This is useful to prove that the element we removed is greater than all the elements left in hte heap
...
6) ~Interm~ is equal to ~H.A~ to the exception of index ~Child.Value~, which is equal to ~V~. This loop_Invariant helps the solvers keep track of the differences between ~Interm~ and ~H.A~
