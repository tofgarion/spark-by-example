...
package Chap5 with
   ...
   function Equal_Range
     (A   : T_Arr;
      Val : T) return Index_Pair is
     (Index_Pair'(Lower_Bound (A, Val), Upper_Bound (A, Val))) with
      Pre =>
      --  XXX GNATProve GPL 2014
      --  For now, we inline definition of Is_Sorted
      (for all J in A'Range =>
      ...
      (if
       ...
       then
         (for all K in A'First .. Equal_Range'Result.First - 1 => A (K) < Val)
         and then
         (for all K in
            Equal_Range'Result.First .. Equal_Range'Result.Second - 1 =>
            A (K) = Val)
         and then
   ...
   function Binary_Search (A : T_Arr; Val : T) return Boolean with
      ...
      (for all J in A'Range =>
         (for all K in J + 1 .. A'Last => A (J) <= A (K))),
      Post =>
      (if Binary_Search'Result then (for some I in A'Range => A (I) = Val));
